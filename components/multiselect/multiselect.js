export default () => ({
  ...
  init() {
    ...
    this.originalSelect = this.$el.querySelector('select');
    this.originalSelect.classList.add('hidden');
    const originalSelect = this.originalSelect;

    const wrapper = this.$el;
    wrapper.className = style.wrapper;

    this.generatedSelect = document.createElement('div');
    this.generatedSelect.className = style.select;
    this.generatedSelect.setAttribute('x-bind', 'selectTrigger');

    this.textList = document.createElement('span');
    this.textList.className = style.textList;
    ...
    this.generatedSelect.append(this.textList);
    this.generatedSelect.append(countBadge);
    this.generatedSelect.append(triggerBtn);
    ...
    const itemGroups = this.originalSelect.querySelectorAll('optgroup');

    if (itemGroups.length > 0) {
      itemGroups.forEach((itemGroup) => processItems(itemGroup, menu));
    } else {
      processItems(this.originalSelect, menu);
    }

    function processItems(parent, menu) {
    ...
    wrapper.appendChild(this.generatedSelect);
  ...
  regenerateTextItems() {
    ...
    if (this.selectedItems.length === 0) {
      this.textList.innerHTML =
        '<span class="text-neutral-400">select</span>';
    } else {
      this.textList.innerText = this.selectedItems.join(', ');
    }
  },
